...
public class Validator implements InitializingBean { 
	...
	public String toJsCode(ValidateResult validateResult) throws UnsupportedEncodingException{
		switch(validateResult.getRunAt()){
			case Js:
			case All:{
	...
	private String toJsCode(ExpTreePart part) throws UnsupportedEncodingException{
		...
		if(partType == PartType.Function){
			...
			return name + "(" + toJsCode(part.getAllChildParts(), part.getFunctionSetting()) +")";
		}
		else if(partType == PartType.Bracket){
			return "(" + toJsCode(part.getAllChildParts(), part.getFunctionSetting()) +")";
		}
	...
	//增加参数ingore的判断 modified by ls 20230512
	private String toJsCode(List<ExpTreePart> allParts, FunctionSetting functionSetting) throws UnsupportedEncodingException{
		if(allParts == null){
			return "";
		}
		else if(functionSetting == null){
			...
			List<Parameter> parameters = functionSetting.getAllParameters();
			List<String> strList = new ArrayList<String>();
			List<Boolean> isParamList = new ArrayList<Boolean>();
			List<String> parameterStrList = new ArrayList<String>();
			for(int i = 0; i < allParts.size();i++){
				ExpTreePart p = allParts.get(i);
				String s = this.toJsCode(p);
				strList.add(s);
				if(p.getPartType() != PartType.Comma){
					parameterStrList.add(s);
					isParamList.add(true);
				}
				else{
					isParamList.add(false);
				}
			}
			int paramIndex = 0;
			StringBuilder ss = new StringBuilder();
			for(int i = 0; i < strList.size();i++){
				ExpTreePart p = allParts.get(i);
				boolean isParam = isParamList.get(i);
				if(isParam){
					Parameter parameter = parameters.get(paramIndex);
					String ignoreMark = parameter.getIgnoreMark();
					if(ignoreMark != null && ignoreMark.length() > 0){
						boolean isNot = ignoreMark.startsWith("!");
						int checkParamIndex = Integer.parseInt(isNot ? ignoreMark.substring(1) : ignoreMark);
						String checkParamStr = parameterStrList.get(checkParamIndex);
						String checkStr = isNot ? ("!(" + checkParamStr + ")") : checkParamStr;
						strList.set(i, "(" + checkStr + " ? null : " + strList.get(i) + ")");
					}
				}
				String s = strList.get(i);
				ss.append(s);
	...
	public ValidateResult validateExp(String exp, List<Parameter> userParameters, RunAt runAt, String needResultType) throws Exception{
		return this.validateExp(exp, userParameters, runAt, needResultType, null);
	}
	
	public ValidateResult validateExp(String exp, List<Parameter> userParameters, RunAt runAt, String needResultType, List<String> functionCategories) throws Exception{
		...
		//判断函数类型是否在允许的范围内 added by ls 20220905
		if(functionCategories != null){
			for(int i = 0; i < allParts.size(); i++){
				ExpTreePart part = allParts.get(i);
				if(part.getPartType() == PartType.Function){
					String functionName = part.getText();
					Function function = this.expEnvironment.getFunction(functionName);
					if(!functionCategories.contains(function.getCategory())){
						errors.add("不允许使用函数" + functionName);
					}
				}
			}
		}
		...
		this.getAllUsedParameters(allParts, usedParameters);
				
		ValidateResult result = new ValidateResult();
		result.setErrors(errors); 
		result.setValueType(resultClientType); 
		result.setUsedParameters(usedParameters);
		if(errors.size()==0){
	...
	private void getAllUsedParameters(List<ExpTreePart> allParts, List<String> usedParameters){
		if(allParts != null && allParts.size() > 0){
			for(int i = 0; i < allParts.size(); i++){
			 	ExpTreePart part = allParts.get(i);
				if(part.getPartType() == PartType.UserParameter && !usedParameters.contains(part.getText())){
					usedParameters.add(part.getText());
				}
				this.getAllUsedParameters(part.getAllChildParts(), usedParameters);
			}
		}
	}
		...
		while(i < expLength){
			...
			if(inStr){ //如果现在正在字符串中	
				...
				if(chr.equals(",")){ //如果遇到了双引号，那么字符串开始
					if(tempStr.length() != 0){
						ExpTreePart p = createPart(tempStr.trim(), PartType.Unknown, null);
						allParts.add(p);
						tempStr = "";
					}
					ExpTreePart p = createPart(chr.trim(), PartType.Comma, null);
					allParts.add(p); 
					i++; 
					continue;
				}
					...
					ExpTreePart p = createPart(chr.trim(), PartType.Bracket, null);
					allParts.add(p); 
					i++;
					continue;
				}		
					...
					ExpTreePart p = createPart(chr.trim(), PartType.Operator, null);
					allParts.add(p); 
					i++;
					continue;
				}	
					...
					if(expLength > i+1){
						...
						if(nextChr.equals("=")){		
							...
							ExpTreePart p = createPart((chr + nextChr).trim(), PartType.Operator, null);
							allParts.add(p); 
							i=i+2;
							continue;
						}
