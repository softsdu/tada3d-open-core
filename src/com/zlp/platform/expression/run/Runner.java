...
import com.zlp.platform.expression.definition.Function;
import com.zlp.platform.expression.definition.FunctionSetting;
import com.zlp.platform.expression.definition.IEnvironment;
import com.zlp.platform.expression.definition.Parameter; 
import com.zlp.platform.expression.definition.PartType;
...
public class Runner {
	...
	public Object runFunction(ExpTreePart part, Map<String, Object> p2vs) throws Exception {
	    ...
	    if(allParameters != null){
	    	...
	    	int childPartCount = childParts == null ? 0 : childParts.size();
	    	...
	    	for(int i = 0; i < funcSettingParamCount; i++){ 
    			...
    			//判断是否忽略此参数 modified by ls 20230421
    			boolean ingore = this.getIgnoreParameter(parameter.getIgnoreMark(), parameterValues);
				...
				if(!repeatable){
					parameterValues[i] = ingore ? null : getResultValue(childP, p2vs);
				}
				else {
	...
	//根据ingoreMark和其他参数，返回某个参数是否被忽略掉 added by ls 20230421
	private boolean getIgnoreParameter(String ignoreMark, Object[] parameterValues){
		if(ignoreMark == null || ignoreMark.length() == 0){
			return false;
		}
		else{
			boolean isNot = ignoreMark.startsWith("!");			
			int paramIndex = Integer.valueOf(isNot ? ignoreMark.substring(1) : ignoreMark);
			Object paramValue = parameterValues[paramIndex];
			if(paramValue == null){
				return false;
			}
			else{
				boolean ignore = isNot ? paramValue.equals(false) : paramValue.equals(true);
				return ignore;
			}
		}
	}
